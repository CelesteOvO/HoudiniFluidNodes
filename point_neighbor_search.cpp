...
void PointSimpleListSearch3::for_each_nearby_point(const UT_Vector3 &origin, const PointNeighborSearch3::ForEachNearbyPointFunc &callback)
{
...
auto PointSimpleListSearch3::has_nearby_point(const UT_Vector3 &origin) const -> bool
{
	fpreal radius_squared = _radius * _radius;
...
void PointSimpleListSearch3::build(const std::vector<UT_Vector3> &points)
{
...
void PointHashGridSearch3::for_each_nearby_point(const UT_Vector3 &origin, const PointNeighborSearch3::ForEachNearbyPointFunc &callback)
{
	if (_buckets.empty())
		return;
...
auto PointHashGridSearch3::has_nearby_point(const UT_Vector3 &origin) const -> bool
{
	if (_buckets.empty())
		return false;
...
void PointHashGridSearch3::build(const std::vector<UT_Vector3> &points)
{
	_buckets.clear();
	_points.clear();
...
void PointHashGridSearch3::add_point(const UT_Vector3 &point)
{
	if (_buckets.empty())
	{
...
auto PointHashGridSearch3::_get_hash_key_from_position(const UT_Vector3 &position) const -> size_t { return _get_hash_key_from_bucket_index(_get_bucket_index(position)); }
auto PointHashGridSearch3::_get_nearby_keys(const UT_Vector3 &position) const -> std::array<size_t, 8>
{
	...
	if ((static_cast<fpreal>(origin_index.x()) + 0.5) * _grid_spacing <= position.x())
	{
	...
	if ((static_cast<fpreal>(origin_index.y()) + 0.5) * _grid_spacing <= position.y())
	{
	...
	if ((static_cast<fpreal>(origin_index.z()) + 0.5) * _grid_spacing <= position.z())
	{
...
auto PointHashGridSearch3::_get_hash_key_from_bucket_index(const UT_Vector3i &index) const -> size_t
{
...
auto PointHashGridSearch3::_get_bucket_index(const UT_Vector3 &position) const -> UT_Vector3i
{
	UT_Vector3i res;
...
void PointParallelHashGridSearch3::for_each_nearby_point(const UT_Vector3 &origin, const PointNeighborSearch3::ForEachNearbyPointFunc &callback)
{
	auto nearby_keys = _get_nearby_keys(origin);
...
auto PointParallelHashGridSearch3::has_nearby_point(const UT_Vector3 &origin) const -> bool
{
	auto nearby_keys = _get_nearby_keys(origin);
	const fpreal query_radius_squared = _radius * _radius;
...
void PointParallelHashGridSearch3::build(const std::vector<UT_Vector3> &points)
{
	if (points.empty())
		return;
...
auto PointParallelHashGridSearch3::_get_hash_key_from_position(const UT_Vector3 &position) const -> size_t { return _get_hash_key_from_bucket_index(_get_bucket_index(position)); }
auto PointParallelHashGridSearch3::_get_nearby_keys(const UT_Vector3 &position) const -> std::array<size_t, 8>
{
	auto origin_index = _get_bucket_index(position);
	std::array<UT_Vector3i, 8> nearby_bucket_indices;
...
auto PointParallelHashGridSearch3::_get_hash_key_from_bucket_index(const UT_Vector3i &index) const -> size_t
{
	UT_Vector3i wrapped_index = index;
...
auto PointParallelHashGridSearch3::_get_bucket_index(const UT_Vector3 &position) const -> UT_Vector3i
{
	UT_Vector3i res;
	res.x() = static_cast<int>(std::floor(position.x() / _grid_spacing));
	res.y() = static_cast<int>(std::floor(position.y() / _grid_spacing));
